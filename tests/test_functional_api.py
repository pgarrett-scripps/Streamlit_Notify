...
class TestFunctionalAPI:
    ...
    def test_notification_functions(self):
        ...
        def app_script_individual_functions():
            import streamlit as st
            import streamlit_notify as stn
            from streamlit_notify.functional import success_stn, error_stn, warning_stn, info_stn
            ...
            if st.button("Check counts", key="check_btn"):
                success_notifications = stn.get_notifications(notification_type=["success"])
                error_notifications = stn.get_notifications(notification_type=["error"])
                warning_notifications = stn.get_notifications(notification_type=["warning"])
